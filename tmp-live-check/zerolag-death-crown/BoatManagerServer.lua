...
local computeDeathPenaltySnapshot: any
local getMinimumRelicLevelForUser: any
...
resolveRespawnTemplate = function(player: Player, fallbackTemplate: any?): any
	...
	userData = UserDataService.Get(player.UserId) or userData
	...
	if typeof(userData) == "table" and typeof(userData.boat) == "table" then
		...
		if typeof(templateId) == "string" and templateId ~= "" then
			local explicitTemplate = BoatTemplates.GetTemplate(templateId)
			if explicitTemplate then
...
computeDeathPenaltySnapshot = function(player: Player, snapshot: { [string]: any }?): { [string]: any }
	local expCurrent = math.max(0, math.floor(tonumber(snapshot and snapshot.expCurrent) or 0))
	local expLevel = math.max(1, math.floor(tonumber(snapshot and snapshot.expLevel) or ExperienceConfig.GetLevelForExp(expCurrent)))
	local minimumRelicLevel = getMinimumRelicLevelForUser(player)
	local targetLevel = math.max(minimumRelicLevel, expLevel - DEATH_LEVEL_PENALTY, 1)

	local currentLevelExp = ExperienceConfig.GetRequiredExpForLevel(expLevel)
	local targetLevelExp = ExperienceConfig.GetRequiredExpForLevel(targetLevel)
	local targetExp = math.max(0, expCurrent - math.max(0, currentLevelExp - targetLevelExp))

	if targetLevel < #ExperienceConfig.Levels then
		local nextLevelExp = ExperienceConfig.GetRequiredExpForLevel(targetLevel + 1)
		targetExp = math.min(targetExp, math.max(targetLevelExp, nextLevelExp - 1))
	end

	local finalLevel = ExperienceConfig.GetLevelForExp(targetExp)
	return {
		expCurrent = targetExp,
		expLevel = finalLevel,
		lostLevels = math.max(0, expLevel - finalLevel),
	}
end

getMinimumRelicLevelForUser = function(player: Player): number
	if typeof(player) ~= "Instance" or not player:IsA("Player") then
		return 1
	end

	local userData = UserDataService.Get(player.UserId)
	if typeof(userData) ~= "table" or typeof(userData.relics) ~= "table" then
		return 1
	end

	local equippedRelicId = userData.relics.equipped
	if typeof(equippedRelicId) ~= "string" or equippedRelicId == "" then
		return 1
	end
	if typeof(userData.relics.unlocked) ~= "table" or userData.relics.unlocked[equippedRelicId] == nil then
		return 1
	end

	local relicTemplate = RelicsService.templates.GetTemplateById(equippedRelicId)
	if relicTemplate == nil then
		return 1
	end

	return math.max(1, math.floor(tonumber(relicTemplate:GetStartLevel()) or 1))
end
...
buildRespawnState = function(userId: number): { [string]: any }
	...
	return {
		...
		lostLevels = math.max(0, math.floor(tonumber(session.lostLevels) or 0)),
	}
end
...
applyDeathRespawnReset = function(player: Player, snapshot: { [string]: any }?)
	local userData = UserDataService.Get(player.UserId)
	if typeof(userData) ~= "table" then
		return
	end

	local penaltySnapshot = computeDeathPenaltySnapshot(player, snapshot)
	local penaltyBoatTemplateId = DEFAULT_RESPAWN_BOAT_ID
	local levelDef = ExperienceConfig.Levels[penaltySnapshot.expLevel]
	if typeof(levelDef) == "table" and typeof((levelDef :: any).BoatTemplateId) == "string" and (levelDef :: any).BoatTemplateId ~= "" then
		penaltyBoatTemplateId = (levelDef :: any).BoatTemplateId
	end

	local okSetExp, setExpErr = ExperienceService.SetExp(player, penaltySnapshot.expCurrent, "death_penalty")
	if okSetExp ~= true then
		warn("[BoatManagerServer] Death penalty exp update failed:", setExpErr)
		local expData = userData.exp
		if typeof(expData) == "table" then
			expData.current = penaltySnapshot.expCurrent
			expData.level = penaltySnapshot.expLevel
		end
	...
	if typeof(boatData) ~= "table" then
		userData.boat = {}
		boatData = userData.boat
	end
	boatData.templateId = penaltyBoatTemplateId

	pcall(function()
		(UserDataManagerServer :: any).SaveNow(player.UserId)
	end)
...
scheduleFullRespawn = function(player: Player, _reason: string, fallbackTemplate: any?)
	...
	local snapshot = captureRespawnSnapshot(player, currentBoat)
	local deathPenaltySnapshot = computeDeathPenaltySnapshot(player, snapshot)
	...
	respawnSessionByUserId[userId] = {
		...
		snapshot = snapshot,
		lostLevels = deathPenaltySnapshot.lostLevels,
	...
	applyDeathRespawnReset(player, snapshot)
